...
app.post('/api/todos', [authJwt.verifyToken,authJwt.isExist,authcontroller.access],async (req, res) => {
    try {
...
app.put('/api/todos/:id',[authJwt.verifyToken,authJwt.isExist,authcontroller.access], async (req, res) => {
    const todo = await Todo.findById(req.params.id);
    if (!todo) {
        return res.status(404).send('Tâche non trouvée');
    }
